...
from eval.eval_script import eval_math
...
def do_parallel_sampling(args, task, answer_extraction_fn, eval_fn, input_dir, output_dir, log_dir):
    ...
    agg_li = []
    for i in range(n_procs):
        agg_li += [json.loads(x) for x in open(f"{output_dir.strip('/')}/results_{i}.jsonl")]
    
    import jsonlines
    with jsonlines.open(f"{output_dir.strip('/')}/results_combined.jsonl", "w") as writer:
        writer.write_all(agg_li)
    
    all_labels = []
    for item in agg_li:
        all_labels.append(eval_math(item))

    with open(os.path.join(output_dir.strip('/'), "summary.json"), "w") as fout:
        json.dump({
            "n_samples": len(all_labels),
            "accuracy": all_labels.count(True) / len(all_labels)
        }, fout, indent=4)
